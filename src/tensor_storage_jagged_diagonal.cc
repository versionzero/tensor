...
void
tensor_storage_convert_from_coordinate_to_jagged_diagonal(tensor_t *destination, tensor_t *source)
{
  int                         n, nnz;
  ...
  d = STORAGE_JAGGED_DIAGONAL(destination);
  
  debug("tensor_storage_convert_from_coordinate_to_jagged_diagonal(destination=0x%x, source=0x%x)\n", destination, source);
...
tensor_storage_malloc_jagged_diagonal(tensor_t const *tensor)
{
  ...
  debug("tensor_storage_malloc_jagged_diagonal: rn=%d, kn=%d\n", storage->rn, storage->kn);
  ...
  case orientation::tube:
    ...
    break;
  default:
    die("tensor_storage_malloc_jagged_diagonal: "
  ...
  superfluous("tensor_storage_malloc_jagged_diagonal: callbacks=0x%x\n", callbacks);
  superfluous("tensor_storage_malloc_jagged_diagonal: storage->CO=0x%x\n", storage->CO);
  superfluous("tensor_storage_malloc_jagged_diagonal: storage->KO=0x%x\n", storage->KO);
  superfluous("tensor_storage_malloc_jagged_diagonal: storage->size (of RO)=%d\n", storage->rn);
  superfluous("tensor_storage_malloc_jagged_diagonal: storage->RO=0x%x\n", storage->RO);
  superfluous("tensor_storage_malloc_jagged_diagonal: storage=0x%x\n", storage);
