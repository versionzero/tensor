...
void
tensor_storage_convert_from_coordinate_to_compressed_fiber_inplace(tensor_t *destination, tensor_t *source)
{
  uint                        nnz;
  ...
  debug("tensor_storage_convert_from_coordinate_to_compressed_fiber_inplace(destination=0x%x, source=0x%x)\n", destination, source);
...
tensor_storage_malloc_compressed_fiber(tensor_t const *tensor)
{
  tensor_storage_base_t       *base;
  tensor_storage_compressed_t *storage;
  conversion_callbacks_t      *callbacks;
  
  superfluous("tensor_storage_malloc_compressed_fiber(tensor=0x%x)\n", tensor);
  ...
  debug("tensor_storage_malloc_compressed_fiber: rn=%d, cn=%d, kn=%d\n", 
  ...
  case orientation::tube:
    ...
    break;
  default:
    die("tensor_storage_malloc_compressed_fiber: "
  ...
  superfluous("tensor_storage_malloc_compressed_fiber: callbacks=0x%x\n", callbacks);
  superfluous("tensor_storage_malloc_compressed: storage->CO=0x%x\n", storage->CO);
  superfluous("tensor_storage_malloc_compressed: storage->KO=0x%x\n", storage->KO);
  superfluous("tensor_storage_malloc_compressed_fiber: storage->size (of RO)=%d\n", storage->rn);
  superfluous("tensor_storage_malloc_compressed_fiber: storage->RO=0x%x\n", storage->RO);
  superfluous("tensor_storage_malloc_compressed_fiber: storage=0x%x\n", storage);
